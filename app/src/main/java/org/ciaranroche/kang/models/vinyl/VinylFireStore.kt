...
class VinylFireStore(val context: Context) : VinylStore {
    ...
    fun updateImage(vinyl: VinylModel) {
        fetchVinyls { }
        if (vinyl.image != "") {
            val fileName = File(vinyl.image)
            val imageName = fileName.getName()

            var imageRef = st.child(appId + '/' + imageName)
            val baos = ByteArrayOutputStream()
            val bitmap = readImageFromPath(context, vinyl.image)
